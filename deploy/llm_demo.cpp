...
//
// =============================================================================
// 说明：本文件为 rkllm-toolkit 的 llm_demo 原版，改动仅在 main() 中新增
// 可选采样参数解析（原版采样参数写死）：
//
...
//
// 前 3 个为必填，其余可选；不传则使用原版默认值（保持兼容）。
// 重新编译后替换 /usr/local/bin/llm_demo 即可让 WebUI 的采样参数真正生效。
// =============================================================================
...
int main(int argc, char **argv)
{
    ...
    // ===== 可选采样参数（原版写死，WebUI 需要可按位置传入覆盖）=====
    if (argc > 4) param.temperature = std::atof(argv[4]);
    if (argc > 5) param.top_p = std::atof(argv[5]);
    if (argc > 6) param.top_k = std::atof(argv[6]);
    if (argc > 7) param.repeat_penalty = std::atof(argv[7]);
    if (argc > 8) param.frequency_penalty = std::atof(argv[8]);
    if (argc > 9) param.presence_penalty = std::atof(argv[9]);
    // ======================================================================
